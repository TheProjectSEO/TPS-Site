...
export function generateTravelGuideSchema(data: TravelGuideSchemaData): BaseStructuredData[] {
  ...
  const currentUrl = `${siteUrl}/travel-guide/${data.slug}`

  // Determine the primary schema type
  const schemaType = data.structured_data_type || 'Article'
  ...
  const articleSchema: BaseStructuredData = {
    ...
    image: data.featured_image ? {
      ...
      description: `${data.title} - Travel Guide Image`
    } : undefined,
    author: {
      '@type': 'Person',
  ...
  if (data.itinerary_days && data.itinerary_days.length > 0) {
    const itinerarySchema: BaseStructuredData = {
      ...
      description: `Detailed ${data.itinerary_days.length}-day itinerary for ${data.destination}`,
      url: currentUrl,
      about: {
        '@type': 'Place',
        name: data.destination
